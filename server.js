...
app.get('/api/admin/dashboard', async (req, res) => {
  ...
  try {
    ...
    // NPS calculation
    const { rows: promotersRows } = await client.query('SELECT COUNT(*) FROM reviews WHERE business_id = $1 AND rating >= 9', [business_id]);
    const { rows: detractorsRows } = await client.query('SELECT COUNT(*) FROM reviews WHERE business_id = $1 AND rating <= 6', [business_id]);
    const total = parseInt(totalRows[0].count, 10) || 0;
    const promoters = parseInt(promotersRows[0].count, 10) || 0;
    const detractors = parseInt(detractorsRows[0].count, 10) || 0;
